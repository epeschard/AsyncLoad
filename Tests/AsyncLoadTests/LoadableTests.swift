...
final class LoadableTests: XCTestCase {
    ...
    func test_cancelLoading() {
        ...
        XCTAssertEqual(cancelBag1.subscriptions.count, 1)
        sut1.cancelLoading()
        XCTAssertEqual(cancelBag1.subscriptions.count, 0)
        XCTAssertNotNil(sut1.error)
        var sut2 = Loadable<Int>.isLoading(last: 7, cancelBag: cancelBag2)
        XCTAssertEqual(cancelBag2.subscriptions.count, 1)
        sut2.cancelLoading()
        XCTAssertEqual(cancelBag2.subscriptions.count, 0)
